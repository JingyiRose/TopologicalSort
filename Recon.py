# Recon.py
# Defines classes related to reconciliations
...
class Node:
    def __init__(self, name=None):
        ...
        self.is_root = False    # Boolean:  True iff this is the root
...
class Tree:
    ...
    def set_leaves(self):
        ...
        self.leaves = self.set_leaves_helper(self.rootNode)

    def set_leaves_helper(self, root):
        if root.is_leaf: return [root]
        else: 
            list1 = self.set_leaves_helper(root.leftChild)
            list2 = self.set_leaves_helper(root.rightChild)
            list1.extend(list2)
            return list1
